...
class TestAudioParser:
    ...
    def test_start_command_detection(self, mock_recognizer, mock_model):
        ...
        with patch("aiteacher.audio.audio_parser.Path.exists", return_value=True):
            ...
            audio_chunk = np.random.normal(0, 0.1, 1600).astype(
                np.float32
            )  # 0.1 seconds

            status, audio, status_changed = parser.add_chunk(audio_chunk)
    ...
    def test_stop_command_detection(self, mock_recognizer, mock_model):
        ...
        with patch("aiteacher.audio.audio_parser.Path.exists", return_value=True):
            ...
            status, audio, status_changed = parser.add_chunk(audio_chunk1)
            assert status == "listening"
            ...
            mock_recognizer_instance.AcceptWaveform.return_value = (
            ...
            mock_recognizer_instance.AcceptWaveform.return_value = True
            mock_recognizer_instance.Result.return_value = '{"text": "stop stop"}'

            audio_chunk4 = np.random.normal(0, 0.1, 1600).astype(np.float32)
            status, returned_audio, status_changed = parser.add_chunk(audio_chunk4)

            assert status == "waiting"
            assert returned_audio is not None
            assert status_changed is True
            assert len(returned_audio) > 0
            # Should contain all chunks from start to stop without start one (?)
    ...
    def test_buffered_duration_calculation(self, mock_recognizer, mock_model):
        ...
        with patch("aiteacher.audio.audio_parser.Path.exists", return_value=True):
            ...
            assert (
                abs(parser.buffered_duration - 0.0) < 0.01
            )  # Should be close to 0 second
    ...
    with (
        ...
        for i in range(0, len(audio_data), chunk_size):
            ...
            if audio_interval is not None:
                speech_intervals.append(audio_interval)
                print(
                    f"Captured speech interval: {len(audio_interval) / sample_rate:.2f} seconds"
                )
