...
def _simulate_wait() -> None:
    ...
    while datetime.now() - start < timedelta(seconds=3):
        _ = np.zeros([3000, 3000]) @ np.zeros([3000, 3000])
    time.sleep(1)


def _stub_wav_processor(
    input_stream: AbstractAudioInputStream, output_dir: str = "out"
) -> None:
    ...
    def _save_chunk(chunk: np.ndarray | None) -> None:
        ...
        if chunk is not None and len(chunk) > 0:
            ...
            print(
                f"Saved audio chunk: {output_path} ({len(chunk) / input_stream.sample_rate:.2f}s)"
            )

            duration = len(chunk) / input_stream.sample_rate
            buffer_duration = input_stream.get_buffer_duration()
            print(
                f"Processed chunk {i}: {duration:.2f}s, buffer: {buffer_duration:.2f}s"
            )
