...
class VoiceConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=False,
        description="Enable voice interface (hands-free mode)"
    )
    ...
    voice_activation_threshold: float = Field(
        default=0.5,
        description="Voice activation detection threshold (0.0-1.0)"
    )
...
class StatisticsConfig(BaseModel):
    ...
    track_errors: bool = Field(
        default=True,
        description="Track errors made during practice"
    )
    
    track_vocabulary: bool = Field(
        default=True,
        description="Track new vocabulary encountered"
    )
    
    export_to_anki: bool = Field(
        default=True,
        description="Enable Anki export functionality"
    )
