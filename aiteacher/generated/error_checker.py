...
class GrammarError(BaseModel):
    ...
    incorrect_text: str = Field(description="The incorrect text from user input")
    ...
    severity: str = Field(
        default="medium", description="Error severity: 'minor', 'medium', 'major'"
    )
...
class VocabularyItem(BaseModel):
    ...
    word: str = Field(description="The word in target language")

    translation: str = Field(description="Translation to native language")

    context: str = Field(description="Context/example sentence where word was used")

    difficulty: str = Field(
        default="medium", description="Word difficulty: 'easy', 'medium', 'hard'"
    )
...
class ErrorCheckResult(BaseModel):
    ...
    vocabulary: List[VocabularyItem] = Field(
        default_factory=list, description="New vocabulary items used"
    )

    overall_feedback: str = Field(description="General feedback on the response")

    fluency_score: Optional[float] = Field(
        default=None, description="Fluency score from 0.0 to 1.0"
    )
